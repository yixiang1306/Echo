...
contextBridge.exposeInMainWorld('audioManagerAPI', {
  playAudio: (audioBase64: string) => ipcRenderer.send('play-audio', audioBase64),
  ...
  resumeAudio: () => ipcRenderer.send('resume-audio'),
    ...
    onPlayAudio: (callback: (audioBase64: string) => void) => ipcRenderer.on("play-audio", (_, audioBase64) => callback(audioBase64)),
    onStopAudio: (callback: () => void) => ipcRenderer.on("stop-audio", () => callback()),
